...
#define LOGF(...)                                          \
    do                                                     \
    {                                                      \
        snprintf(logf_buf, sizeof(logf_buf), __VA_ARGS__); \
...
static void game_restart(u32 width, u32 height)
{
    ...
    // I believe snprintf should be only used for LOGF and in the "release" build snprintf should not be included at all
    snprintf(game.score_buffer, sizeof(game.score_buffer), "Score: %u", game.score);
}
